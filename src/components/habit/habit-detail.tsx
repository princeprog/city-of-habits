    ...
    <div className="min-h-[calc(100vh-3.5rem)] px-4 py-6 sm:px-6 sm:py-10 lg:px-10">
      <div className="mx-auto max-w-6xl">
        ...
        <header className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-end">
          ...
          <Card size="sm"><CardHeader><CardDescription>This week</CardDescription><CardTitle>{weeklyCount}<span className="ml-1">/ {habit.targetPerWeek}</span></CardTitle><CardDescription>{habitCheckIns.length} total</CardDescription><Progress value={Math.min(100, Math.round((weeklyCount / habit.targetPerWeek) * 100))} aria-label={`${weeklyCount} of ${habit.targetPerWeek} this week`} /></CardHeader></Card>
        </header>
        ...
        <Tabs defaultValue="history" className="mt-8">
          ...
          <TabsContent value="history" className="mt-4 grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
            ...
            <div className="flex flex-col gap-4"><Card><CardHeader><CardTitle>Recent notes</CardTitle></CardHeader><CardContent>{habitCheckIns.some((checkIn) => checkIn.note) ? <ItemGroup>{habitCheckIns.filter((checkIn) => checkIn.note).slice(0, 4).map((checkIn) => <Item key={checkIn.id} size="sm" variant="muted"><ItemContent><ItemDescription>{checkIn.note}</ItemDescription><ItemDescription>{checkIn.localDate}</ItemDescription></ItemContent></Item>)}</ItemGroup> : <p className="text-sm text-muted-foreground">Add context on a future check-in when it feels useful.</p>}</CardContent></Card><Card><CardHeader><CardDescription>Lifetime</CardDescription><CardTitle>{habitCheckIns.length}</CardTitle><CardDescription>days that belong to this story.</CardDescription></CardHeader></Card></div>
          </TabsContent>
